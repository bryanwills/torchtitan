    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for param in group["params"]:
                ...
                # Gather in bf16: NS casts its input to bf16 as its first op, and the fp32->bf16
                # cast commutes with the lossless all-gather, so this is bit-identical to
                # gathering the fp32 update while halving the gathered bytes (the fp32 master/
                # grad would otherwise move at 4 B/elem). The momentum state stays fp32.
                # 2D matrix -> one NS; 3D [E, m, n] MoE expert stack -> batched per-expert NS,
                # with the lr adjusted by the per-expert (m, n) shape.
